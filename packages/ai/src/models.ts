 ...
 * chain-of-thought before emitting output. With a small `maxOutputTokens` budget
 ...
 * generation we don't need deep reasoning, so we pass `reasoningEffort: 'low'`
 * via providerOptions to keep the token budget available for the response itself.
 */
...
export const structuredProviderOptions = {
  openai: { reasoningEffort: "low" as const, strictJsonSchema: true },
};
